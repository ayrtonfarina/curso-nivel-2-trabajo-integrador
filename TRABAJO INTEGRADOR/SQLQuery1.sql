...
select A.Id, Codigo, Nombre, A.Descripcion, M.Descripcion Marca, C.Descripcion Categoria, ImagenUrl, Precio, A.IdCategoria From ARTICULOS A, MARCAS M, CATEGORIAS C Where M.Id = A.IdMarca and C.Id = A.IdCategoria 

update ARTICULOS set Codigo = @codigo, Nombre = @nombre, Descripcion = @desc, IdMarca = idMarca, IdCategoria = idCat, ImagenUrl = @img, Precio = @precio where Id = @id 


delete from ARTICULOS where Id = 6

SELECT A.Id, Codigo, Nombre, A.Descripcion, M.Descripcion as Marca, C.Descripcion as Categoria, ImagenUrl, Preio, A.IdMarca, A.IdCategoria FROM ARTICULOS A, MARCAS M, CATEGORIAS C Where A.IdMarca = M.Id AND A.IdCategoria = C.Id 
